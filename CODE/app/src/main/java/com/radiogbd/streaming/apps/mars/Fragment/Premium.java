...
public class Premium extends Fragment implements ObservableScrollViewCallbacks {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        albumBanner.setImageDrawable(ContextCompat.getDrawable(context, R.drawable.preimum));
        getAllPremiumAlbum(0);
    ...
    private void getAllPremiumAlbum(int page) {
        if(utility.isNetworkAvailable()) {
            //String album = "list_album-" + page;
            //utility.logger(album);
            utility.showProgress(false);
            Call<List<AlbumModel>> call = apiInterface.getPremiumList(getString(R.string.authorization_key), subscriptionBox.getOperator(), subscriptionBox.getMobile(), utility.getFirebaseToken(), String.valueOf(page));
                    ...
                    if (response.isSuccessful() && response.code() == 200) {
                        try {
                            ...
                            if (albumModels.size() > 0) {
                                if(albumModels.size()>4&&albumModels.size()<10){
                                    albumModels.add(new AlbumModel(0));
                                    albumModels.add(new AlbumModel(0));
                                }
